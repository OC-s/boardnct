	...
	<div class="container">
		<table class="table table-striped">
			<form action="search" method="get" name="search-form">
				<thead>
					<tr>
						<td><a href="write" class="btn btn-outline-primary">작성</a></td>
				...
				<tbody>
					<c:forEach var="dto" items="${list}">
						<tr>
							<td><span>${dto.bno }</span></td>
							<td><span>${dto.nickname }</span></td>
							<td><a href="detail?bno=${dto.bno}">${dto.title }</a></td>
							<td><span>${dto.contents }</span></td>
							<td><span>${dto.created_time }</span></td>
						</tr>
					</c:forEach>
								...
								<ul class="pagination justify-content-center"
											...
											<c:choose>
												<c:when test="${empty keyword}">
													<li class="page-item"><a class="page-link"
														href="search?type=${type}&keyword=${keyword}&cp=${map.currentPage-5 }">이전</a></li>
												</c:when>
												<c:otherwise>
													<li class="page-item"><a class="page-link"
														href="search?type=${type}&keyword=${keyword}&cp=${map.currentPage-5 }">이전</a></li>
									...
									<c:forEach var="i" begin="${map.startPage }"
										...
										<c:choose>
											<c:when test="${empty keyword}">
												<li class="page-item"><a class="page-link"
													href="search?type=${type}&keyword=${keyword}&cp=${i}">${i }</a></li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="search?type=${type}&keyword=${keyword}&cp=${i}">${i }</a></li>
									...
									<c:choose>
										<c:when test="${map.isNext }">
											<c:choose>
												<c:when test="${empty keyword}">
													<li class="page-item"><a class="page-link"
														href="search?type=${type}&keyword=${keyword}&cp=${map.currentPage+5 }">다음</a></li>
												</c:when>
												<c:otherwise>
													<li class="page-item"><a class="page-link"
														href="search?type=${type}&keyword=${keyword}&cp=${map.currentPage+5 }">다음</a></li>
